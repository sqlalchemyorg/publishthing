...
def github_hook(
    ...
    def create_new_gerrit_for_review_request(
        ...
        with thing.shell_in(workdir).shell_in(owner, create=True) as shell:
            ...
            gh_repo.publish_pr_comment_w_status_change(
                ...
                event.json_data["pull_request"]["head"]["sha"],
                (
                    "New Gerrit review created"
                    if is_new_gerrit
                    else "Patchset added to existing Gerrit review"
                ),
                state="success",
                context="gerrit_review",
    ...
    @thing.github_webhook.event(
        "pull_request_review", util.github_pr_review_is_submitted
    )
    def mirror_pr_reviews(
        ...
        if github_user in set(
            ...
            thing.debug(
                ...
                "not mirroring comment / pullrequest",
                github_user,
            )
            return
        ...
        for gerrit_revision_sha, gerrit_revision in all_revisions[
            "revisions"
        ].items():
            pullreq_match = util.get_pullreq_for_gerrit_commit_message(
                event.repo_name, gerrit_revision["commit"]["message"]
            )
        ...
        for comment in comments_to_post:
            gerrit_file_position = pullreq_index.convert_github_line_position(
                util.GithubReviewPosition(comment["path"], comment["position"])
            )
        ...
        message = util.format_github_comment_for_gerrit(github_user, message)

        review = {"message": message, "comments": inline_comments}
    ...
    @thing.github_webhook.event(
        "issue_comment", util.github_comment_is_pullrequest
    )
